...
export default function TransportationPage() {
  const [transportation, setTransportation] = useState(['No', 'No']);
  const router = useRouter();

  const handleSelect = (guestIdx: number, value: 'Yes' | 'No') => {
    setTransportation(prev => prev.map((v, i) => (i === guestIdx ? value : v)));
  };
              ...
              <div className="flex flex-col w-full items-start">
                ...
                <div className="flex flex-row gap-4 w-full">
                  {['Yes', 'No'].map(option => (
                    <button
                      key={option}
                      className={`w-full py-3 rounded-md text-base transition-colors focus:outline-none ${transportation[0] === option ? 'bg-[#08080A] text-white' : 'bg-[#F5F5F5] text-[#08080A]'}`}
              ...
              <div className="flex flex-col w-full items-end">
                ...
                <div className="flex flex-row gap-4 w-full justify-end">
                  {['Yes', 'No'].map(option => (
                    <button
                      key={option}
                      className={`w-full py-3 rounded-md text-base transition-colors focus:outline-none ${transportation[1] === option ? 'bg-[#08080A] text-white' : 'bg-[#F5F5F5] text-[#08080A]'}`}
                      style={{ fontFamily: 'Montserrat', fontSize: '15px' }}
